...
class World:
    ...
    # Returns [[items_found],[items_in_inventory],[[open_item_locations],[open_ability_locations]]]
    def traverse(self, start_items=[],print_log=False):
        self.unsolve()
        self.update_graph(True,False,False,start_items,print_log)
    ...
    def check_edge(self,edge,items=[],test=False):
    ...
    # Returns progression list if applicable, -1 if no progression found, and -2 if progression found but inventory space is too limited
    ...
    def lock_dark_spaces(self):
        ...
        for edge in self.logic:
            for req in self.logic[edge][3]:
                if req[0] == 600 and self.logic[edge][1] not in nodes:
                    nodes.append(self.logic[edge][1])

        for node in nodes:
            #print(self.graph[node][5])
    ...
    def check_ds_access(self, start_node=-1, need_freedan=False,items=[]):
        ...
        if self.graph[start_node][4] == 2 or (self.graph[start_node][4] == 1 and not need_freedan):
            ...
            self.update_graph(True,True,False,items)
...
#        print(self.exits[origin_exit][10], "-", self.exits[dest_exit][10])
        if dest not in self.graph[origin][1] and self.exits[origin_exit][5]:
            self.graph[origin][1].append(dest)
        if origin not in self.graph[dest][8] and self.exits[origin_exit][5]:
            self.graph[dest][8].append(origin)
...
#        print("Progression exits done")
        ...
        while origin_exits:
            ...
            # If we run out of exits, loosen restrictions
            if not origin_exits:
                origin_exits += quarantine
                quarantine.clear()
                if check_ds_access and check_direction:
                    check_direction = False
                elif check_ds_access and not check_direction:
                    check_ds_access = False
                    check_direction = True
                elif check_progression and check_direction:
                    check_direction = False
                elif check_progression and not check_direction:
...
#        print("Round 3 exits done")
    ...
    def update_graph(self,update_logic=True,update_ds=True,update_exits=False,items=[],print_log=False):
        ...
        for node in self.graph:
            for x in self.graph[node][1]:
                if node not in self.graph[x][8]:
                    self.graph[x][8].append(node)
                ...
                while to_visit:
                    ...
                    if self.graph[node][4] < access_mode or access_mode == 2:
                        ...
                        for x in self.graph[node][8]:
                            if self.consider_ds_node(x,access_mode) and ds_node not in self.graph[x][9] and x not in to_visit:
                                to_visit.append(x)
                                if print_log:
                                    print(" -Discovered:",self.graph[x][5])
...
#            print(x,self.graph[x])
    ...
    def initialize(self):
        ...
        if "Allow Glitches" in self.variant:
            ...
            self.logic[268][3][1][1] = 0          # Ankor Wat: Earthquaker not required
            self.logic[273][3][0][1] = 0          # Ankor Wat: Glasses not required
            self.logic[274][3][0][1] = 0
        ...
        if "Open Mode" in self.variant:
            # Update graph logic
            self.logic[30][3][0][1] = 0     # Lola's Letter
            self.logic[31][3][0][1] = 0
            self.logic[32][3][0][1] = 0
            self.logic[33][3][0][1] = 0     # Memory Melody
            self.logic[36][3][0][1] = 0     # Teapot
            self.logic[38][3][0][1] = 0     # Will
            self.logic[39][3][0][1] = 0
            self.logic[40][3][0][1] = 0     # Roast
...
#            print("Boss order: ",self.boss_order)
    ...
    def randomize(self, seed_adj=0):
        ...
        # Fill the Mystic Statues
        self.fill_statues()

        # Lock Dark Spaces for form changes in Completable mode
...
#                print(self.overworld_menus)
            ...
            open_ability_locations = traverse_result[1][1]
...
#        self.level = settings.level.value
        ...
        # World graph
        # Format: { Region ID: Traversed_flag, [AccessibleRegions], type(0=other/misc,1=exterior,2=interior), [continentID,areaID,layer,MapID],
        #                 DS_access(0=no_access,1=any_DS,2=form_change_DS), RegionName, [ItemsToRemove], ForceFormChange, [AccessibleFromRegions], [Accessible_DSs]}
        self.graph = {
            # Game Start
            0: [False, [22,600], 0, [0,0,0,b"\x00"], 0, "Game Start", [], True, [], []],  # 600 shouldn't be here, fix later
            ...
            57: [False, [],    2, [1,3,0,b"\x00"], 0, "Itory: Cave (behind false wall)", [], False, [], []],
            ...
            62: [False, [],       2, [1,4,0,b"\x00"], 0, "Moon Tribe: Cave (Pedestal)", [], False, [], []],
            ...
            137: [False, [],    2, [2,8,0,b"\x00"], 0, "Diamond Mine: Map 64 (trapped laborer)", [], False, [], []],
            138: [False, [],    2, [2,8,0,b"\x00"], 0, "Diamond Mine: Map 65 (main)", [], False, [], []],
            139: [False, [],    2, [2,8,0,b"\x00"], 0, "Diamond Mine: Map 65 (behind ramp)", [], False, [], []],
            ...
            145: [False, [],    2, [2,8,0,b"\x00"], 0, "Diamond Mine: Map 67 (exit)", [], False, [], []],         # potential softlock?
            ...
            # Sky Garden
            168: [False, [],         2, [2,10,0,b"\x00"], 0, "Sky Garden: Map 81 (north)", [], False, [], []],
            ...
            183: [False, [],         2, [2,10,0,b"\x00"], 0, "Sky Garden: Map 81 (Dark Space cage)", [], False, [], []],
            184: [False, [183],      2, [2,10,0,b"\x00"], 0, "Sky Garden: Map 81 (SE platform)", [], False, [], []],
            185: [False, [183],      2, [2,10,0,b"\x00"], 0, "Sky Garden: Map 81 (SW platform)", [], False, [], []],
            ...
            189: [False, [507],      2, [2,10,0,b"\x00"], 0, "Sky Garden: Map 82 (switch cage)", [], False, [], []],
            ...
            206: [False,    [], 2, [3,11,0,b"\x00"], 0, "Seaside Palace: Buffy", [], False, [], []],
            207: [False,    [], 2, [3,11,0,b"\x00"], 0, "Seaside Palace: Area 2 SW Room", [], False, [], []],
            208: [False,    [], 2, [3,11,0,b"\x00"], 0, "Seaside Palace: Coffin", [], False, [], []],
            ...
            273: [False,    [], 2, [3,13,0,b"\x00"], 0, "Angel Dungeon: Map 116 (Ishtar's chest)", [], False, [], []],
            ...
            288: [False,    [280], 0, [3,14,0,b"\x00"], 0, "Watermia: Stablemaster", [], False, [], []],
            ...
            311: [False, [310], 0, [4,16,0,b"\x00"], 0, "Euro: Stablemaster", [], False, [], []],
            ...
            315: [False, [],    2, [4,16,0,b"\x00"], 0, "Euro: Ann", [], False, [], []],
            ...
            351: [False, [350], 0, [4,18,0,b"\x00"], 0, "Natives' Village: Child Guide", [], False, [], []],
            ...
            377: [False, [],    2, [4,19,0,b"\x00"], 0, "Ankor Wat: Map 184 (N)", [], False, [], []],
            ...
            412: [False, [],        2, [5,21,0,b"\x00"], 0, "Pyramid: Entrance (hidden platform)", [], False, [], []],
            ...
            450: [False, [],        2, [5,21,0,b"\x00"], 0, "Pyramid: Map 215 (past Killer 6)", [], False, [], []],
            ...
            492: [False,    [], 0, [0,0,0,b"\x00"], 0, "Dark Gaia/End Game", [], False, [], []],
        ...
        # Format: { ID: [Status(0=locked,1=unlocked,-1=restricted), StartRegion, DestRegion, [[item1, qty1],[item2,qty2]...]]}
        self.logic = {
            # Jeweler Rewards
            0: [0, 1, 2, [[1, gem[0]]]],  # Jeweler Reward 1
            1: [0, 1, 2, [[1, gem[0] - 2], [41, 1]]],
            2: [0, 1, 2, [[1, gem[0] - 3], [42, 1]]],
            3: [0, 1, 2, [[1, gem[0] - 5], [41, 1], [42, 1]]],
            4: [0, 2, 3, [[1, gem[1]]]],  # Jeweler Reward 2
            5: [0, 2, 3, [[1, gem[1] - 2], [41, 1]]],
            6: [0, 2, 3, [[1, gem[1] - 3], [42, 1]]],
            7: [0, 2, 3, [[1, gem[1] - 5], [41, 1], [42, 1]]],
            8: [0, 3, 4, [[1, gem[2]]]],  # Jeweler Reward 3
            9: [0, 3, 4, [[1, gem[2] - 2], [41, 1]]],
            10: [0, 3, 4, [[1, gem[2] - 3], [42, 1]]],
            11: [0, 3, 4, [[1, gem[2] - 5], [41, 1], [42, 1]]],
            12: [0, 4, 5, [[1, gem[3]]]],  # Jeweler Reward 4
            13: [0, 4, 5, [[1, gem[3] - 2], [41, 1]]],
            14: [0, 4, 5, [[1, gem[3] - 3], [42, 1]]],
            15: [0, 4, 5, [[1, gem[3] - 5], [41, 1], [42, 1]]],
            16: [0, 5, 6, [[1, gem[4]]]],  # Jeweler Reward 5
            17: [0, 5, 6, [[1, gem[4] - 2], [41, 1]]],
            18: [0, 5, 6, [[1, gem[4] - 3], [42, 1]]],
            19: [0, 5, 6, [[1, gem[4] - 5], [41, 1], [42, 1]]],
            20: [0, 6, 7, [[1, gem[5]]]],  # Jeweler Reward 6
            21: [0, 6, 7, [[1, gem[5] - 2], [41, 1]]],
            22: [0, 6, 7, [[1, gem[5] - 3], [42, 1]]],
            23: [0, 6, 7, [[1, gem[5] - 5], [41, 1], [42, 1]]],
            24: [0, 7, 8, [[1, gem[6]]]],  # Jeweler Reward 7 (Mansion)
            25: [0, 7, 8, [[1, gem[6] - 2], [41, 1]]],
            26: [0, 7, 8, [[1, gem[6] - 3], [42, 1]]],
            27: [0, 7, 8, [[1, gem[6] - 5], [41, 1], [42, 1]]],

            # Inter-Continental Travel
            30: [0, 28,   15, [[37, 1]]],   # South Cape: Erik w/ Lola's Letter
            31: [0, 102,  15, [[37, 1]]],   # Coast: Turbo w/ Lola's Letter
            32: [0, 280,  15, [[37, 1]]],   # Watermia: Bridgeman w/ Lola's Letter
            33: [0, 160, 161, [[13, 1]]],   # Neil's: Neil w/ Memory Melody
            34: [0, 314,  17, [[505, 1]]],  # Euro: Neil w/ Memory restored
            35: [0, 402,  17, [[505, 1]]],  # Dao: Neil w/ Memory restored
            36: [0,  60,  64, [[25, 1]]],   # Moon Tribe healed w/ Teapot
            37: [0, 170,  16, [[502, 1]]],  # Sky Garden: Spirits w/ spirits healed
            38: [0, 280, 288, [[24, 1]]],   # Watermia: Stablemaster w/ Will
            39: [0, 310, 311, [[24, 1]]],   # Euro: Stablemaster w/ Will
            40: [0, 350, 351, [[10, 1]]],   # Natives': Child Guide w/ Large Roast

            # Edward's / Tunnel
            60: [0, 32, 33, [[2, 1]]],    # Escape cell w/Prison Key
            61: [0, 33, 32, [[2, 1]]],    # Enter cell w/Prison Key
            62: [0, 45, 46, [[501, 1]]],  # Progression w/ Lilly
            63: [0, 47, 48, [[600, 1]]],  # Activate Bridge w/ Freedan

            # Itory
            70: [0, 50, 51, [[9, 1]]],    # Town appears w/ Lola's Melody
            71: [0, 55, 59, [[23, 1]]],   # Get Lilly w/ Necklace
            72: [0, 56, 57, [[61, 1]]],   # Cave w/ Psycho Dash
            73: [0, 56, 57, [[62, 1]]],   # Cave w/ Psycho Slide
            74: [0, 56, 57, [[63, 1]]],   # Cave w/ Spin Dash

            # Moon Tribe
            80: [0, 61, 62, [[61, 1]]],   # Cave challenge w/ Psycho Dash
            81: [0, 61, 62, [[62, 1]]],   # Cave challenge w/ Psycho Slide
            82: [0, 61, 62, [[63, 1]]],   # Cave challenge w/ Spin Dash

            # Inca / Gold Ship / Freejia
            90:  [0, 77, 78, [[3, 1], [4, 1]]],  # Map 30 progression w/ Inca Statues
            91:  [0, 80, 81, [[61, 1]]],         # Map 32 progression w/ Psycho Dash
            92:  [0, 80, 81, [[62, 1]]],         # Map 32 progression w/ Psycho Slider
            93:  [0, 80, 81, [[63, 1]]],         # Map 32 progression w/ Spin Dash
            94:  [0, 85, 86, [[600, 1]]],        # Map 35 progression w/ Freedan
            95:  [0, 87, 88, [[8, 1]]],          # Map 36 progression w/ Wind Melody
            96:  [0, 89, 90, [[7, 1]]],          # Map 37 progression w/ Diamond Block
            97:  [0, 91, 92, [[61, 1]]],         # Map 38 progression w/ Psycho Dash
            98:  [0, 91, 92, [[62, 1]]],         # Map 38 progression w/ Psycho Slider
            99:  [0, 91, 92, [[63, 1]]],         # Map 38 progression w/ Spin Dash
            100: [0, 100, 104, [[100, 1]]],      # Gold Ship progression w/ Statue 1
            101: [0, 110, 115, [[504, 1]]],      # Freejia: Slaver item w/ Laborer Found

            # Diamond Mine
            110: [0, 131, 132, [[61, 1]]],            # Map 61 false wall w/ Psycho Dash
            111: [0, 131, 132, [[62, 1]]],            # Map 61 false wall w/ Psycho Slider
            112: [0, 131, 132, [[63, 1]]],            # Map 61 false wall w/ Spin Dash
            113: [0, 134, 135, [[15, 1]]],            # Map 63 progression w/ Elevator Key
            114: [0, 136, 137, [[61, 1]]],            # Map 64 trapped laborer w/ Psycho Dash
            115: [0, 136, 137, [[62, 1]]],            # Map 64 trapped laborer w/ Psycho Slider
            116: [0, 136, 137, [[63, 1]]],            # Map 64 trapped laborer w/ Spin Dash
            117: [0, 138, 139, [[63, 1]]],            # Map 65 progression w/ Spin Dash
            118: [0, 138, 139, [[64, 1], [600, 1]]],  # Map 65 progression w/ Dark Friar
            119: [0, 146, 147, [[11, 1], [12, 1]]],   # Map 68 progression w/ mine keys

            # Sky Garden
            130: [0, 170, 171, [[14, 4]]],            # Boss access w/ Crystal Balls
            131: [0, 177, 178, [[64, 1], [600, 1]]],  # Map 79 progression w/ Dark Friar
            132: [0, 177, 178, [[67, 1], [600, 1]]],  # Map 79 progression w/ Firebird
            133: [0, 168, 182, [[506, 1]]],           # Map 81 progression w/ switch 1
            134: [0, 182, 183, [[507, 1]]],           # Map 81 progression w/ switch 2
            135: [0, 182, 184, [[61, 1]]],            # Map 81 progression w/ Psycho Dash
            136: [0, 182, 184, [[62, 1]]],            # Map 81 progression w/ Psycho Dash
            137: [0, 182, 184, [[63, 1]]],            # Map 81 progression w/ Psycho Dash
            138: [0, 184, 185, [[508, 1], [61, 1]]],  # Map 81 progression w/ switch 3 & Psycho Dash
            139: [0, 184, 185, [[508, 1], [62, 1]]],  # Map 81 progression w/ switch 3 & Psycho Slider
            140: [0, 184, 185, [[508, 1], [63, 1]]],  # Map 81 progression w/ switch 3 & Spin Dash
            141: [0, 181, 182, [[63, 1]]],            # Map 81 progression w/ Spin Dash
            142: [0, 181, 184, [[63, 1]]],            # Map 81 progression w/ Spin Dash
            143: [0, 182, 185, [[63, 1]]],            # Map 81 progression w/ Spin Dash
            144: [0, 188, 189, [[600, 1]]],           # Map 82 progression w/ Freedan
            145: [0, 188, 189, [[601, 1]]],           # Map 82 progression w/ Glitches
            146: [0, 192, 190, [[63, 1]]],            # Map 83 progression w/ Spin Dash
            147: [0, 195, 199, [[64, 1], [600, 1]]],  # Map 84 progression w/ Dark Friar
            148: [0, 195, 199, [[67, 1], [600, 1]]],  # Map 84 progression w/ Firebird
            149: [0, 195, 199, [[65, 1], [600, 1]]],  # Map 84 progression w/ Aura Barrier
            150: [0, 197, 199, [[64, 1], [600, 1]]],  # Map 84 progression w/ Dark Friar
            151: [0, 197, 199, [[67, 1], [600, 1]]],  # Map 84 progression w/ Firebird
            152: [0, 170,  16, [[502, 1]]],           # Moon Tribe passage w/ spirits healed

            # Seaside Palace
            160: [0, 205, 208, [[501, 1]]],   # Coffin access w/ Lilly
            161: [0, 209, 510, [[17, 1]]],    # Purify fountain w/stone
            162: [0, 200, 206, [[510, 1]]],   # Buffy access w/ purified fountain
            163: [0, 200, 201, [[16, 1]]],    # Seaside to Mu w/ Mu key
            164: [0, 210, 211, [[16, 1]]],    # Mu to Seaside w/ Mu key

            # Mu
            170: [0, 212, 245, [[62, 1]]],                       # Map 95 progression w/ Psycho Slider
            171: [0, 212, 213, [[511, 1]]],                      # Map 95 progression w/ water lowered 1
            172: [0, 213, 215, [[512, 1]]],                      # Map 95 progression w/ water lowered 2
            173: [0, 214, 216, [[512, 1]]],                      # Map 95 progression w/ water lowered 2
            174: [0, 217, 218, [[511, 1]]],                      # Map 96 progression w/ water lowered 1
            175: [0, 222, 221, [[511, 1], [64, 1], [600, 1]]],   # Map 97 progression w/ water lowered 1 & Friar
            176: [0, 222, 221, [[511, 1], [67, 1], [600, 1]]],   # Map 97 progression w/ water lowered 1 & Firebird
            177: [0, 222, 221, [[511, 1], [601, 1]]],            # Map 97 progression w/ water lowered 1 & glitches
            178: [0, 226, 227, [[511, 1]]],                      # Map 98 progression w/ water lowered 1
            179: [0, 227, 229, [[512, 1]]],                      # Map 98 progression w/ water lowered 2
            180: [0, 228, 230, [[512, 1]]],                      # Map 98 progression w/ water lowered 2
            181: [0, 229, 230, [[62, 1]]],                       # Map 98 progression w/ Psycho Slider
            182: [0, 230, 229, [[62, 1]]],                       # Map 98 progression w/ Psycho Slider
            183: [0, 226, 246, [[62, 1]]],                       # Map 98 progression w/ Psycho Slider
            184: [0, 237, 238, [[62, 1]]],                       # Map 101 progression w/ Psycho Slider
            185: [0, 240, 241, [[19, 2]]],                       # Map 102 progression w/ Rama Statues
            186: [0, 231, 247, [[18, 1]]],                       # Water lowered 1 w/ Hope Statue
            187: [0, 232, 248, [[18, 2]]],                       # Water lowered 2 w/ Hope Statues

            # Angel Dungeon
            210: [0, 263, 264, [[62, 1]]],    # Map 112 progression w/ Psycho Slider
            211: [0, 265, 275, [[62, 1]]],    # Map 112 backwards progression w/ Psycho Slider
            212: [0, 267, 268, [[62, 1]]],    # Map 114 progression w/ Psycho Slider
            213: [0, 277, 276, [[62, 1]]],    # Map 114 backwards progression w/ Psycho Slider
            214: [0, 272, 273, [[513, 1]]],   # Ishtar's chest w/ puzzle complete

            # Great Wall
            220: [0, 294, 295, [[601, 1]]],           # Map 133 progression w/ glitches
            221: [0, 296, 295, [[63, 1]]],            # Map 133 progression w/ Spin Dash
            222: [0, 296, 295, [[600, 1]]],           # Map 133 progression w/ Freedan
            223: [0, 298, 299, [[64, 1], [600, 1]]],  # Map 135 progression w/ Friar
            224: [0, 298, 299, [[67, 1], [600, 1]]],  # Map 135 progression w/ Firebird
            225: [0, 299, 298, [[64, 1], [54, 2]]],   # Map 135 progression w/ Friar III
            227: [0, 300, 301, [[63, 1]]],            # Map 136 progression w/ Spin Dash
            228: [0, 295, 294, [[63, 1]]],            # Map 133 progression w/ Spin Dash

            # Euro
            230: [0, 314, 315, [[40, 1]]],    # Ann item w/ Apple

            # Mt. Temple
            240: [0, 331, 332, [[63, 1]]],   # Map 161 progression w/ Spin Dash
            241: [0, 332, 331, [[63, 1]]],   # Map 161 backwards progression w/ Spin Dash
            242: [0, 333, 514, [[26, 1]]],   # Map 162 progression w/ Mushroom drops 1
            243: [0, 335, 514, [[26, 1]]],   # Map 162 progression w/ Mushroom drops 1  -- IS THIS TRUE?
            244: [0, 339, 515, [[26, 2]]],   # Map 162 progression w/ Mushroom drops 2
            245: [0, 340, 515, [[26, 2]]],   # Map 162 progression w/ Mushroom drops 2  -- IS THIS TRUE?
            246: [0, 340, 516, [[26, 3]]],   # Map 162 progression w/ Mushroom drops 3
            247: [0, 341, 516, [[26, 3]]],   # Map 162 progression w/ Mushroom drops 3  -- IS THIS TRUE?

            # Natives'
            250: [0, 353, 354, [[29, 1]]],    # Statues awake w/ Gorgon Flower

            # Ankor Wat
            #260: [0, 361, 362, [[64, 1], [600, 1]]],            # Map 177 progression w/ Friar
            261: [0, 363, 364, [[63, 1]]],                      # Map 178 progression w/ Spin Dash
            262: [0, 364, 365, [[62, 1]]],                      # Map 178 progression w/ Psycho Slider
            263: [0, 365, 364, [[62, 1]]],                      # Map 178 progression w/ Psycho Slider
            264: [0, 367, 366, [[63, 1]]],                      # Map 179 progression w/ Spin Dash
            265: [0, 369, 370, [[62, 1]]],                      # Map 181 progression w/ Psycho Slider
            266: [0, 370, 371, [[63, 1]]],                      # Map 181 progression w/ Spin Dash
            267: [0, 373, 374, [[66, 1], [600, 1]]],            # Map 183 progression w/ Earthquaker
            268: [0, 373, 374, [[64, 1], [54, 2], [600, 1]]],   # Map 183 progression w/ upgraded Friar
            269: [0, 373, 374, [[64, 1], [601, 1], [600, 1]]],  # Map 183 progression w/ Friar and glitches
            270: [0, 373, 374, [[67, 1], [600, 1]]],            # Map 183 progression w/ Firebird       -- IS THIS TRUE?
            271: [0, 376, 377, [[64, 1], [600, 1]]],            # Map 184 progression w/ Friar
            272: [0, 376, 377, [[36, 1], [600, 1]]],            # Map 184 progression w/ Shadow
            273: [0, 384, 385, [[28, 1], [62, 1]]],             # Map 188 progression w/ Black Glasses & Slider
            274: [0, 385, 384, [[28, 1], [62, 1]]],             # Map 188 progression w/ Black Glasses & Slider
            275: [0, 384, 385, [[601, 1], [62, 1]]],            # Map 188 progression w/ glitches & Slider
            276: [0, 385, 384, [[601, 1], [62, 1]]],            # Map 188 progression w/ glitches & Slider
            277: [0, 386, 387, [[62, 1]]],                      # Map 188 progression w/ Psycho Slider
            278: [0, 387, 386, [[62, 1]]],                      # Map 188 progression w/ Psycho Slider

            # Pyramid
            290: [0, 410, 411, [[62, 1]]],             # Map 204 progression w/ Slider
            291: [0, 410, 411, [[63, 1]]],             # Map 204 progression w/ Spin
            292: [0, 410, 411, [[601, 1]]],            # Map 204 progression w/ glitches
            293: [0, 411, 412, [[36, 1]]],             # Map 204 progression w/ Aura
            294: [0, 411, 413, [[36, 1]]],             # Map 204 progression w/ Aura
            295: [0, 415, 449, [[30, 1], [31, 1], [32, 1], [33, 1], [34, 1], [35, 1], [38, 1]]],
                                                    # Boss door open w/ Hieroglyphs
            296: [0, 416, 417, [[63, 1]]],             # Map 206 progression w/ Spin Dash
            297: [0, 417, 416, [[63, 1]]],             # Map 206 progression w/ Spin Dash
            298: [0, 418, 419, [[63, 1]]],             # Map 206 progression w/ Spin Dash
            299: [0, 419, 418, [[63, 1]]],             # Map 206 progression w/ Spin Dash
            300: [0, 426, 427, [[36, 1], [600, 1]]],   # Map 212 progression w/ Aura
            301: [0, 426, 427, [[66, 1], [600, 1]]],   # Map 212 progression w/ Earthquaker
            302: [0, 427, 428, [[36, 1], [600, 1]]],   # Map 212 progression w/ Aura
            303: [0, 427, 429, [[36, 1], [600, 1]]],   # Map 212 progression w/ Aura
            304: [0, 427, 429, [[66, 1], [600, 1]]],   # Map 212 progression w/ Earthquaker
            305: [0, 431, 432, [[63, 1]]],             # Map 214 progression w/ Spin Dash
            306: [0, 431, 434, [[36, 1], [600, 1]]],   # Map 214 progression w/ Aura
            307: [0, 431, 433, [[64, 1], [600, 1]]],   # Map 214 progression w/ Friar
            308: [0, 438, 439, [[63, 1]]],             # Map 217 progression w/ Spin Dash
            309: [0, 439, 438, [[63, 1]]],             # Map 217 progression w/ Spin Dash
            310: [0, 440, 441, [[63, 1]]],             # Map 219 progression w/ Spin Dash
            311: [0, 441, 440, [[63, 1]]],             # Map 219 progression w/ Spin Dash
            312: [0, 435, 450, [[6, 6], [50, 2], [51, 1], [52, 1]]],
                                                    # Killer 6 w/ herbs and upgrades
            313: [0, 435, 450, [[64, 1], [54, 1], [600, 1]]],
                                                    # Killer 6 w/ Friar II
            314: [0, 411, 414, [[517, 1]]],            # Pyramid to boss w/hieroglyphs placed

            # Babel / Mansion
            320: [0, 461, 462, [[36, 1], [39, 1]]],    # Map 219 progression w/ Aura and Ring
            321: [0, 473, 479, [[522, 1]]],            # Olman statue w/ Mummy Queen 2
            322: [0, 473, 479, [[523, 1]]],            # Olman statue w/ Solid Arm
            323: [0, 480, 481, [[62, 1]]],             # Mansion progression w/ Slider

            # Endgame / Misc
            400: [0, 49, 490, [[20, 2]]],                       # Rescue Kara from Edward's w/ Magic Dust
            401: [0, 150, 490, [[20, 2]]],                      # Rescue Kara from Mine w/ Magic Dust
            402: [0, 270, 490, [[20, 2]]],                      # Rescue Kara from Angel w/ Magic Dust
            403: [0, 345, 490, [[20, 2]]],                      # Rescue Kara from Mt. Temple w/ Magic Dust
            404: [0, 391, 490, [[20, 2]]],                      # Rescue Kara from Ankor Wat w/ Magic Dust
            405: [0, 490, 491, [[36, 1], [39, 1]]],             # Early Firebird w/ Kara, Aura and Ring
            406: [0, 490, 492, [[36, 1], [100, 0], [101, 0], [102, 0], [103, 0], [104, 0], [105, 0]]]
